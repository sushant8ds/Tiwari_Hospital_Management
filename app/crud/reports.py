...
class ReportsCRUD:
    ...
    async def get_collection_summary_report(
        self,
        db: AsyncSession,
        start_date: date,
        end_date: date
        ...
        summary = {
            ...
            "by_type": {
                "OPD_FEE": Decimal("0.00"),
                "IPD_ADVANCE": Decimal("0.00"),
                "INVESTIGATION": Decimal("0.00"),
                "PROCEDURE": Decimal("0.00"),
                "SERVICE": Decimal("0.00"),
                "OT": Decimal("0.00"),
                "DISCHARGE": Decimal("0.00"),
                "MANUAL": Decimal("0.00")
            },
            "by_mode": {
                "CASH": Decimal("0.00"),
        ...
        for payment in payments:
            pay_date = payment.payment_date.strftime("%Y-%m-%d")
            amount = payment.amount
            pay_type = payment.payment_type.value if hasattr(payment.payment_type, "value") else str(payment.payment_type)
            pay_mode = payment.payment_mode.upper()
            
            summary["total_collection"] += amount
            if pay_type in summary["by_type"]:
                summary["by_type"][pay_type] += amount
            else:
                summary["by_type"][pay_type] = summary["by_type"].get(pay_type, Decimal("0.00")) + amount
                
            if pay_mode in summary["by_mode"]:
                summary["by_mode"][pay_mode] += amount
            else:
                summary["by_mode"][pay_mode] = summary["by_mode"].get(pay_mode, Decimal("0.00")) + amount
            
            if pay_date not in daily_trends:
                daily_trends[pay_date] = {
                    "date": pay_date,
                    "total": Decimal("0.00"),
                    "by_type": {t: Decimal("0.00") for t in summary["by_type"]},
                    "by_mode": {m: Decimal("0.00") for m in summary["by_mode"]}
                }
            
            daily_trends[pay_date]["total"] += amount
            if pay_type in daily_trends[pay_date]["by_type"]:
                daily_trends[pay_date]["by_type"][pay_type] += amount
            if pay_mode in daily_trends[pay_date]["by_mode"]:
                daily_trends[pay_date]["by_mode"][pay_mode] += amount
                
        formatted_summary = {
            "total_collection": float(summary["total_collection"]),
            "by_type": {k: float(v) for k, v in summary["by_type"].items()},
            "by_mode": {k: float(v) for k, v in summary["by_mode"].items()}
        }
        
        formatted_trends = []
        for trend_date in sorted(daily_trends.keys()):
            trend = daily_trends[trend_date]
            formatted_trends.append({
                "date": trend["date"],
                "total": float(trend["total"]),
                "by_type": {k: float(v) for k, v in trend["by_type"].items()},
                "by_mode": {k: float(v) for k, v in trend["by_mode"].items()}
            })
