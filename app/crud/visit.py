...
class VisitCRUD:
    ...
    async def create_visit(
        ...
        if visit_type == VisitType.EMERGENCY:
            opd_fee = Decimal("500.00")
        elif visit_type == VisitType.OPD_NEW:
        ...
        try:
            ...
            db.add(visit)
            # Flush first to ensure visit exists for foreign key constraint in payment
            await db.flush()
            ...
            await payment_crud.create_payment(
                ...
                patient_id=patient_id,
                amount=opd_fee,
                payment_mode=payment_mode.value if hasattr(payment_mode, 'value') else str(payment_mode),
