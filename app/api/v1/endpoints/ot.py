...
@router.get("/tomorrow")
async def get_tomorrow_ot_schedule(
    db: AsyncSession = Depends(get_db)
):
    """Get tomorrow's OT schedule (Public endpoint for dashboard)"""
    try:
        procedures = await ot_crud.get_tomorrow_ot_procedures(db)
        results = []
        for proc in procedures:
            results.append({
                "ot_id": proc.ot_id,
                "ipd_id": proc.ipd_id,
                "patient_name": proc.ipd.patient.name if proc.ipd and proc.ipd.patient else "N/A",
                "bed_number": proc.ipd.bed.bed_number if proc.ipd and proc.ipd.bed else "N/A",
                "operation_name": proc.operation_name,
                "surgeon_name": proc.surgeon_name,
                "operation_date": proc.operation_date.isoformat(),
