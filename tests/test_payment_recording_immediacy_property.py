...
class TestPaymentRecordingImmediacy:
    ...
    async def test_balance_reflects_charges_and_payments_immediately(
        ...
        # Since the OPD fee was already paid on registration, balance due is just the charge_amount
        assert balance_after_charge["balance_due"] == charge_amount
        ...
        assert balance_after_payment["total_paid"] == Decimal(str(visit.opd_fee)) + payment_amount
        assert balance_after_payment["balance_due"] == charge_amount - payment_amount
    ...
    async def test_visit_payment_history_updated_immediately(
        ...
        # Check initial visit payment history (should have exactly 1 auto-payment from registration)
        initial_history = await payment_crud.get_payments_by_visit(
            db=db_session,
            visit_id=visit.visit_id
        )
        assert len(initial_history) == 1
        assert initial_history[0].amount == Decimal(str(visit.opd_fee))
        ...
        history_ids = [p.payment_id for p in updated_history]
        assert payment.payment_id in history_ids
