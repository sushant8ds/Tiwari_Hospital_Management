...
class TestBarcodeContentValidation:
    ...
    async def test_barcode_contains_patient_and_visit_id(
        ...
        assert len(barcode_parts) >= 5
        # Since patient ID has the format P-YYMM-XXXX (contains 2 hyphens)
        reconstructed_patient_id = "-".join(barcode_parts[:3])
        assert reconstructed_patient_id == patient.patient_id
        assert barcode_parts[3] == visit.visit_id
        # Last part is timestamp
        assert barcode_parts[4].isdigit()
    ...
    async def test_barcode_contains_patient_and_ipd_id(
        ...
        reconstructed_patient_id = "-".join(barcode_parts[:3])
        assert reconstructed_patient_id == patient.patient_id
        assert barcode_parts[3] == ipd.ipd_id
        assert barcode_parts[4].isdigit()
...
class TestBarcodeRecordMapping:
    ...
    async def test_barcode_maps_to_correct_patient_record(
        ...
        barcode_parts = slip.barcode_data.split("-")
        extracted_patient_id = "-".join(barcode_parts[:3])
        extracted_visit_id = barcode_parts[3]
